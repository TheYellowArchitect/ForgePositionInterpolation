...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] NetworkCommunicationControllerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is NetworkCommunicationControllerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkCommunicationControllerNetworkObject.Length > 0 && NetworkCommunicationControllerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetworkCommunicationControllerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkCommunicationControllerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateNetworkCommunicationController instead, its shorter and easier to type out ;)")]
		public NetworkCommunicationControllerBehavior InstantiateNetworkCommunicationControllerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkCommunicationControllerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkCommunicationControllerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetworkCommunicationControllerBehavior>().networkObject = (NetworkCommunicationControllerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of NetworkCommunicationController
		/// </summary>
		/// <returns>
		/// A local instance of NetworkCommunicationControllerBehavior
		/// </returns>
		/// <param name="index">The index of the NetworkCommunicationController prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public NetworkCommunicationControllerBehavior InstantiateNetworkCommunicationController(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (NetworkCommunicationControllerNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: NetworkCommunicationController. Add them at the NetworkManager prefab.");
				return null;
			}
